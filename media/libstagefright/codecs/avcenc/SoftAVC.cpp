...
static status_t ConvertAvcSpecLevelToOmxAvcLevel(WORD32 avcLevel,
        OMX_VIDEO_AVCLEVELTYPE *omxLevel) {
    for (size_t i = 0; i < NELEM(ConversionTable); ++i) {
...
SoftAVC::SoftAVC(const char *name,
...
OMX_ERRORTYPE SoftAVC::setNumCores() {
    ...
    status = ive_api_function(mCodecCtx, (void *) &s_num_cores_ip,
            (void *) &s_num_cores_op);
    if (status != IV_SUCCESS) {
...
OMX_ERRORTYPE SoftAVC::setProfileParams() {
    ...
    status = ive_api_function(mCodecCtx, &s_profile_params_ip,
            &s_profile_params_op);
    if (status != IV_SUCCESS) {
...
void SoftAVC::logVersion() {
    ...
    status = ive_api_function(mCodecCtx, (void *) &s_ctl_ip,
            (void *) &s_ctl_op);
...
OMX_ERRORTYPE SoftAVC::initEncoder() {
    ...
    case OMX_COLOR_FormatYUV420SemiPlanar:
        ...
        ALOGD("colorFormat YUV_420SP");
        break;
    default:
    case OMX_COLOR_FormatYUV420Planar:
        mIvVideoColorFormat = IV_YUV_420P;
        ALOGD("colorFormat YUV_420P");
        break;
    }
    ...
    mMemRecords = (iv_mem_rec_t *) malloc(
...
OMX_ERRORTYPE SoftAVC::releaseEncoder() {
    ...
    status = ive_api_function(mCodecCtx, &s_retrieve_mem_ip,
            &s_retrieve_mem_op);
...
OMX_ERRORTYPE SoftAVC::internalGetParameter(OMX_INDEXTYPE index,
        OMX_PTR params) {
    switch (index) {
...
OMX_ERRORTYPE SoftAVC::internalSetParameter(OMX_INDEXTYPE index,
    ...
    case OMX_IndexParamVideoAvc: {
        ...
        if (OK
                != ConvertOmxAvcLevelToAvcSpecLevel(avcType->eLevel,
                        &mAVCEncLevel)) {
            return OMX_ErrorUndefined;
        }
...
OMX_ERRORTYPE SoftAVC::setEncodeArgs(
    ...
    if (inputBufferHeader) {
        ...
        if (mInputDataIsMeta) {
            source = extractGraphicBuffer(mConversionBuffer,
                    (mWidth * mHeight * 3 / 2), source,
    ...
    if (inputBufferHeader) {
        ps_encode_ip->u4_timestamp_high = (inputBufferHeader->nTimeStamp)
                >> 32;
        ps_encode_ip->u4_timestamp_low = (inputBufferHeader->nTimeStamp)
                & 0xFFFFFFFF;
    }
...
void SoftAVC::onQueueFilled(OMX_U32 portIndex) {
    ...
    if (mCodecCtx == NULL) {
        if (OMX_ErrorNone != initEncoder()) {
            ...
            notify(OMX_EventError, OMX_ErrorUndefined, 0, // Extra notification data
    ...
    while (!mSawInputEOS && !inQueue.empty() && !outQueue.empty()) {
        ...
        if (!mSpsPpsHeaderReceived) {
            error = setEncodeArgs(&s_encode_ip,
                    &s_encode_op, NULL, outputBufferHeader);
            if (error != OMX_ErrorNone) {
                mSignalledError = true;
                notify(OMX_EventError, OMX_ErrorUndefined, 0, 0);
                return;
            }
            status = ive_api_function(mCodecCtx, &s_encode_ip,
                    &s_encode_op);
