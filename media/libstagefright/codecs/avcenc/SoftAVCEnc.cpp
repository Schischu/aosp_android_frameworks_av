...
OMX_ERRORTYPE SoftAVC::initEncoder() {
    ...
    if (mInputDataIsMeta) {
        for(i = 0; i < MAX_CONVERSION_BUFFERS; i++) {
...
OMX_ERRORTYPE SoftAVC::setEncodeArgs(
    ...
    source = NULL;
    if (inputBufferHeader && inputBufferHeader->nFilledLen) {
...
void SoftAVC::onQueueFilled(OMX_U32 portIndex) {
    ...
    while ( !mSawOutputEOS && !outQueue.empty()) {
        ...
        if (inputBufferHeader && inputBufferHeader->nFlags & OMX_BUFFERFLAG_EOS) {
            mSawInputEOS = true;
        }

        /* In normal mode, store inputBufferInfo and this will be returned
           when encoder consumes this input */
        if (!mInputDataIsMeta && inputBufferInfo) {
            for(i = 0; i < MAX_INPUT_BUFFER_HEADERS; i++) {
        ...
        DUMP_TO_FILE(mInFile, s_encode_ip.s_inp_buf.apv_bufs[0],
                (mHeight * mStride * 3 / 2));
        ...
        if(s_encode_op.s_inp_buf.apv_bufs[0]) {
            if (mInputDataIsMeta) {
                for(i = 0; i < MAX_CONVERSION_BUFFERS; i++) {
