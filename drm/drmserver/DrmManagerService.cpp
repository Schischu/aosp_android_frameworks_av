...
static const char *get_perm_label(drm_perm::drm_perm_t perm) {
    unsigned int index = perm;

    if (index >= (sizeof(drm_perm::drm_perm_labels)
                / sizeof(drm_perm::drm_perm_labels[0]))) {
        ALOGE("SELinux: Failed to retrieve permission label(perm=%d).\n", perm);
        abort();
    }
    return drm_perm::drm_perm_labels[index];
}

static bool selinuxIsProtectedCallAllowed(pid_t spid, drm_perm::drm_perm_t perm) {
...
static bool isProtectedCallAllowed(drm_perm::drm_perm_t perm) {
    // TODO
...
void DrmManagerService::instantiate() {
    ...
    if (getcon(&drmserver_context) != 0 && selinux_enabled > 0) {
...
status_t DrmManagerService::consumeRights(
    ...
    if (!isProtectedCallAllowed(drm_perm::CONSUME_RIGHTS)) {
...
status_t DrmManagerService::setPlaybackStatus(
    ...
    if (!isProtectedCallAllowed(drm_perm::SET_PLAYBACK_STATUS)) {
        return DRM_ERROR_NO_PERMISSION;
    }
...
DecryptHandle* DrmManagerService::openDecryptSession(
    ...
    if (isProtectedCallAllowed(drm_perm::OPEN_DECRYPT_SESSION)) {
...
status_t DrmManagerService::closeDecryptSession(int uniqueId, DecryptHandle* decryptHandle) {
    ...
    if (!isProtectedCallAllowed(drm_perm::CLOSE_DECRYPT_SESSION)) {
        return DRM_ERROR_NO_PERMISSION;
    }
...
status_t DrmManagerService::initializeDecryptUnit(int uniqueId, DecryptHandle* decryptHandle,
    ...
    if (!isProtectedCallAllowed(drm_perm::INITIALIZE_DECRYPT_UNIT)) {
        return DRM_ERROR_NO_PERMISSION;
    }
...
status_t DrmManagerService::decrypt(
    ...
    if (!isProtectedCallAllowed(drm_perm::DECRYPT)) {
        return DRM_ERROR_NO_PERMISSION;
    }
...
status_t DrmManagerService::finalizeDecryptUnit(
    ...
    if (!isProtectedCallAllowed(drm_perm::FINALIZE_DECRYPT_UNIT)) {
        return DRM_ERROR_NO_PERMISSION;
    }
...
ssize_t DrmManagerService::pread(int uniqueId, DecryptHandle* decryptHandle,
    ...
    if (!isProtectedCallAllowed(drm_perm::PREAD)) {
        return DRM_ERROR_NO_PERMISSION;
    }
