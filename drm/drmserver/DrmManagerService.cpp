...
static const char *get_perm_label(drm_perm_t perm) {
    ...
    if (index >= (sizeof(drm_perm_labels) / sizeof(drm_perm_labels[0]))) {
        ALOGE("DrmManagerService: Failed to retrieve permission label. Aborting DrmManagerService.\n");
        abort();
    }
    return drm_perm_labels[index];
}

static bool selinuxIsProtectedCallAllowed(drm_perm_t perm, pid_t spid) {
    if (!selinux_enabled) {
        return true;
    }
...
static bool isProtectedCallAllowed(drm_perm_t perm) {
    ...
    for (unsigned int i = 0; i < trustedUids.size(); ++i) {
        if (trustedUids[i] == uid) {
            return selinuxIsProtectedCallAllowed(perm, spid);
        }
    }
    return false;
}
...
status_t DrmManagerService::consumeRights(
    ...
    if (!isProtectedCallAllowed(android::CONSUME_RIGHTS)) {
...
status_t DrmManagerService::setPlaybackStatus(
    ...
    if (!isProtectedCallAllowed(android::SET_PLAYBACK_STATUS)) {
        return DRM_ERROR_NO_PERMISSION;
    }
...
DecryptHandle* DrmManagerService::openDecryptSession(
    ...
    if (isProtectedCallAllowed(android::OPEN_DECRYPT_SESSION)) {
...
status_t DrmManagerService::closeDecryptSession(int uniqueId, DecryptHandle* decryptHandle) {
    ...
    if (!isProtectedCallAllowed(android::CLOSE_DECRYPT_SESSION)) {
        return DRM_ERROR_NO_PERMISSION;
    }
...
status_t DrmManagerService::initializeDecryptUnit(int uniqueId, DecryptHandle* decryptHandle,
    ...
    if (!isProtectedCallAllowed(android::INITIALIZE_DECRYPT_UNIT)) {
        return DRM_ERROR_NO_PERMISSION;
    }
...
status_t DrmManagerService::decrypt(
    ...
    if (!isProtectedCallAllowed(android::DECRYPT)) {
        return DRM_ERROR_NO_PERMISSION;
    }
...
status_t DrmManagerService::finalizeDecryptUnit(
    ...
    if (!isProtectedCallAllowed(android::FINALIZE_DECRYPT_UNIT)) {
        return DRM_ERROR_NO_PERMISSION;
    }
...
ssize_t DrmManagerService::pread(int uniqueId, DecryptHandle* decryptHandle,
    ...
    if (!isProtectedCallAllowed(android::PREAD)) {
        return DRM_ERROR_NO_PERMISSION;
    }
